...
| A room card says *Uploading…* and never finishes | It gives up after 90 seconds now and turns into *⚠ shot N not saved* with a **Retry**. The photo is still on the device — it is the connection to Google that failed, usually the same cause as a red sync chip bottom-left. Fix that, then press Retry. On a build before 2026-09-11 it would sit there for the rest of the job; reload to pick up the current one. |
...
| A non-estate job's plan looks much shorter than an estate one | Correct, and by design. Every labour service runs the **same four phases**; the estate rigor is *added* on top, not a different plan. Estate Settlement and both probates add *Authority & Legal* in Phase 0, *Chain of Custody* and the *§733.604 Documentation Package* in Phase 1, and probate adds *Florida Court & Legal Compliance* in Phase 4. Measured on one house: Home Editing 35 checkboxes, Home Cleanout 40, Estate Settlement 58, Probate 68. Home Transition is the only one with a fifth phase (Move Day). **Home Prep for Sale is the one genuinely different plan** — no phases, no room cards, no hours log. |
| A room you locked in Phase 1 looks unset in Phase 2 | The lock **did** carry — check the badge in the corner of the card, and the green *locked* button. If the badge reads **PENDING** or **SORTING** then that room genuinely is not locked yet: Phase 2 only draws *locked / packed / complete*, so a room below that lights none of the three. The card says which it is and you can lock it right there. **⚠ One real exception:** anything locked on the OTHER device before the Apps Script redeploy of 2026-09-11 is genuinely gone — re-lock it once and it will hold. |
