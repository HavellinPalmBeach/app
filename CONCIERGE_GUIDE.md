...
>
> **2 · 📷 Items.** Sort the room, then shoot it: one shot per object, or per obvious lot. Pick a chip — **Keep · Donate · Sell · Remove · Undecided** — and it stays lit shot after shot until you change it, so a shelf going to Sell is one tap for the whole shelf. **Undecided is the default**: anything you do not decide in the house lands at the top of the Inventory tab that evening, which is where it belongs. No name, no category, no value — that is desk work.
...
- **🎤 Hold to talk** — hold the button, say it, let go; the words ride the last shot as a note (*"signed on the base, promised to Karen"*). Where the phone cannot dictate there is a box to type it. It shows on that item's row on the Inventory tab.
- **The camera stays open until you press Done.** Shoot the whole room in one go — the count in the corner ticks up, the last shot sits as a thumbnail beside the shutter, and every shot is filed to the client's **Estate Inventory** folder as you go. The phone's own camera app is not used unless the site is refused camera access, in which case the same screen falls back to it one shot at a time.
...
| Where do I type the object's name? | You don't, in the house. Shoot it, chip it, talk the note if there is one. It arrives on the Inventory tab under *Unnamed shots*, and naming it there takes seconds with the photo in front of you. |
