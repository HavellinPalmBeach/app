...
- **On a Home Cleanout, answer the tick box: *"This is the property of someone who has died"*.** It appears on that service and no other. Tick it and the form asks for the deceased's name and the **authorised representative** instead of a client phone and email; leave it for a landlord or an owner clearing their own place.
...
> **⚠** **That tick box is the single most consequential thing on this form, and it is only on Home Cleanout.** It decides how *every* client document is written — the estimate's stages and the engagement agreement both — and there is no way to fix it on the documents afterwards. A cleanout for a family whose mother died last month, saved without it, addresses her as the living owner of her own house, to the people who just buried her. Ten seconds now. The three estate services never ask, because they are decedent work by definition.
...
> **⚠** **What decides which version you get is one question: is the client alive, or is the client dead?** Not the service type on its own. The app reads it as *deceased* if the service is Home Cleanout, Probate or Contested Probate — **or** if you entered a **date of death** at intake — **or** if you entered an **authorised representative**. Any one of the three flips the whole document, and the agreement in step 6 with it.
>
> **If this document has the wrong voice, the fix is at intake, not here.** On a Home Cleanout go back and tick *"This is the property of someone who has died"* (step 1); on anything else check the authorised representative and the date of death. Then re-open the estimate. Do not rewrite the wording — it is regenerated every time anyone renders the document.
...
> **⚠** **There are two agreement forms and you do not choose between them.** The app picks on the same living/deceased question as the estimate (step 3): a **deceased** client gets the **estate form**, written to a representative signing in a fiduciary capacity; a **living** client gets the standard form. **Read the first paragraph before you send it.** If it addresses the wrong kind of client, the problem is at intake — a missing date of death or a missing authorised representative — so go and fix that, then come back and re-generate. Do not edit the agreement text by hand.
...
| The estimate talks to the client as though they were alive, and they are not | On a **Home Cleanout**, the tick box at intake was left unticked — go back and tick it. On anything else, check the **authorised representative** and the **date of death**. Then re-open the estimate. Do not edit the wording; it is rebuilt next time anyone renders it. |
| You cannot find the tick box for a deceased client | It only appears on **Home Cleanout**. Estate Settlement and both probates never show it — they are decedent jobs by definition and already write that way. Downsizing, move and prep never show it either. |
| Intake will not save a ticked cleanout without a representative | Correct. Ticking the box disables the client's own phone and email, so the rep's **last name and a phone or email** become the only way to reach anybody. The rest of that block is optional on a cleanout — you are not being held to probate's requirements. |
| You got the standard agreement on a cleanout for someone who has died | Same cause, same fix — the agreement reads the same rule. Before 2026-08-03 a cleanout *always* got the standard form; if one went out on an estate before then, re-generate it and read it. |
