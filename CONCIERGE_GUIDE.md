...
| A non-estate job's plan looks much shorter than an estate one | Correct, and by design. Every labour service runs the **same four phases**; the estate rigor is *added* on top, not a different plan. Estate Settlement and both probates add the in-house documentation boxes in Phase 1 (firearms photographed where they lie, each checked for NFA, documents sequestered, cash logged) and the *written release authority* line in Phase 2; probate adds the Letters and §733.604 deadline lines in Phase 0 and the *Florida court & legal compliance* list under Job Admin on the Inventory tab. Measured on one six-room house after the 2026-09-19 cut: Home Editing 9 boxes, Home Cleanout 13, Estate Settlement 17, Probate 18 — Home Transition 28, because Move Day is a real list — and most of what used to be a box is now a line the app reads for itself. **Home Prep for Sale is the one genuinely different plan** — no phases, no room list, no hours log. |
| A room reads *Pending* though you have finished it | Nothing marks a room done by itself. Open the room and tap **Lock** (sorted and shot) or **Cleared** (emptied, and the After shot taken). Rooms are drawn once now, above the phases — there is no Phase 2 grid to look at. **⚠ One real exception** still stands: anything locked on the OTHER device before the Apps Script redeploy of 2026-09-11 is genuinely gone — re-lock it once and it will hold. |
