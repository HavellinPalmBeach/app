...
### What each service type's plan actually contains

Asked directly (2026-09-11): *"does the job plan change for estate and non-estate jobs? The estate job plan is very detailed and complicated but obviously needs rigor for PR documentation or court quality — what do non estate jobs have?"* **It is one plan, not two.** Six of the seven services run the same phase spine and the estate rigor is *added* to it; only Home Prep for Sale has a different renderer. Driven on the real Job Plan across all seven service types, same house, same six rooms:

| Service | Phases | Checkboxes | Hours log | What it adds over the one above it |
|---|---|---|---|---|
| **Home Editing** | 4 | 35 | Yes | The baseline. Phase 2 is *Pack & Prepare* — keep / sell / donate / dispose. |
| **Home Transition** | **5** | 53 | Yes | The only service with a fifth phase: **Phase 3 — Move Day** (Pre-Move / Origin / Destination), plus *New Home Prep* in Phase 0. |
| **Home Cleanout** | 4 | 40 | Yes | Phase 2 becomes *Pack & Disposition*: the full streams (haul, donation, consignment, auction, shred), donation receipt, chain-of-custody closed on every pickup. |
| **Estate Settlement** | 4 | 58 | Yes | Phase 0 becomes *Pre-Job & Authority* and gains **Authority & Legal** (Letters, PR authority full vs limited, attorney contact, access authorization, NDA per crew member, COI). Phase 1 becomes *Sort, Decisions & Documentation* and gains **Chain of Custody — Mandatory** and the **§733.604 Documentation Package**. The phase gates reword to the **PR** rather than the client. |
| **Probate** | 4 | 68 | Yes | Phase 4 becomes *Close-Out & Court Filing* and gains **Florida Court & Legal Compliance** (8 items). Phase 0 gains the **60-day §733.604 deadline** calculated from Letters issuance, and the 30–45 day substantial-completion target. |
| **Contested Probate** | 4 | 68 | Yes | **Nothing.** Identical to Probate, section for section — see the note below. |
| **Home Prep for Sale** | — | 6 | **No** | A different document entirely (§6c): vendor sourcing, budget and a coordination checklist. No phases, no room cards, no hours log — that engagement bills no hours. |
...
> **⚠ A room renders TWICE in every phase plan, on purpose.** Phase 1's grid caps a room at *locked* (decisions final) and Phase 2's owns *packed → complete* (execution), so the crew cannot quietly drive a room to complete from Phase 1 and skip the midpoint invoice with it. It is also why the retired print ran to nine pages: 36 cards for 18 rooms.

> **⚠ A room that is not yet locked shows NOTHING lit in Phase 2, and now says why (2026-09-12).** Asked directly: *"why don't the 'locked' from phase 1 carry to starting point in stage 2?"* **They do carry, and that was measured before anything was changed** — both grids read the same stored status, so a room locked in Phase 1 shows *LOCKED* in Phase 2 with the green button lit, immediately and after any redraw. What does not carry is every state *below* locked: Phase 2's row is *locked / packed / complete*, so a room still at *pending* or *sorting* lit none of the three — a blank row of grey buttons, which reads exactly like state that failed to arrive. The status was always there; the button row had no way to say so. Such a card now carries *"Still pending — finish the decisions in Phase 1, or lock it here"*, and the line clears the instant the room locks. **⚠ It flags and never refuses** — locking a room from the Phase 2 card is legitimate, because the crew is standing in it. The phase split exists to stop Phase 1 driving a room to *complete* and skipping the midpoint invoice, not to stop Phase 2 locking one.
