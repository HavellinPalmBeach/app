...
> **⚠ THE FINAL STATES THE VARIANCE AND NEVER ASSERTS THAT ANYBODY WAS TOLD — corrected 2026-09-11.** The Payment Summary row read *"Job ran over estimate by $1,360 — client was notified per T&Cs"* on **every** positive variance, however small. Two things wrong at once, and the second is the worse one. The notice is a **threshold** obligation — the estimate's Terms and the agreement's §3.8 both promise it only *"if actual hours exceed the estimate by more than 15%"* — so on the job that surfaced this, 5.3% over, no notice was owed and none was given. And **the app records no notification at any variance**: that conversation happens mid-job, and what the system can actually evidence is an *accepted change order*, which already has its own section on the same document. So the claim was unverifiable above the threshold too. It now reads *"Job ran over estimate by $1,360 (5.3%) — inside the 15% tolerance in your agreement"*, or *beyond* it past 15%, and the same ±15% constant drives that sentence, the manager PIN and the change-order screen — so a final can never print *inside the tolerance* over an invoice the app is withholding for being outside it. **A fee-only Home Prep job cites no threshold at all**, because its §3.8 states none. **Nothing about what is billed changed.** Re-print any final issued before 2026-09-11.
