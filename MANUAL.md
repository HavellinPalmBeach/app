...
> **⚠ REQUIRES AN APPS SCRIPT REDEPLOY — 2026-09-21a, `saveInventory.gs`.** One project and one deployment, so paste both files and *Deploy → Manage deployments → New version*; `BACKEND_VERSION` in `main-sync.gs` is bumped to `2026-09-21a` because that constant is the only thing the app can read to tell which vintage is serving. What an older deployment does: a **living client’s own workbook goes on printing *Date of Death*, *Letters Issued* and a *§733.604 Inventory Deadline*** over three empty cells, plus an estate-only valuation block and flags (§10a). Nothing is lost and nothing is wrong on an estate job — a payload with no documentation flag on it renders the estate layout deliberately, so that an old browser tab left open cannot strip a court deadline off a live probate matter. **Any living-client workbook written before the redeploy should be re-synced**: open the client on the Job Admin & Inv tab and make any edit, and the sheet is rewritten.
