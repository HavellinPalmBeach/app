...
> **"This is the property of someone who has died" — the tick box on Home Cleanout (new 2026-08-03).** Home Cleanout is the only service that genuinely runs both ways: a landlord or a relocating owner clearing their own property, and just as often the house of somebody who has just died. Nothing else on the form could tell those apart, so **every document was written to a living owner** — including on jobs whose client was the person who had just buried them. Ticking the box switches the form (client name becomes *Deceased full name*, phone and email are disabled) and opens the **Authorized Representative** block, where the rep's **last name and one contact method** are required — with the client's own contact fields disabled they are the only route to anybody. It does *not* pull in probate's mandatory fields. It is the same flag the client estimate and the agreement read (§7, §8), and it is cleared automatically if the service type changes to one that cannot show it.
...
> **ONE RULE DECIDES THE VOICE: is the client alive, or is the client dead?** `isDecedentJob(job)` is the single answer, and both the estimate and the agreement read it. It is true when the **service type** is Home Cleanout, Probate or Contested Probate; or when a **date of death** is recorded; or when an **authorised representative** is recorded. Any one of the three is enough.
>
> **Why it is not just the service type.** A Home Cleanout is routinely the house of somebody who has just died, and it prices no documentation step at all — so the old test (four named service types) addressed a decedent cleanout to a living owner of their own property, and an Estate Settlement job entered as a cleanout got the wrong document throughout. The date of death and the representative are what stop that. Conversely a downsizing with no rep and no date of death is a living client and reads that way, which is correct.
>
> **Living** → it is *your home*, *your things*, and the decisions are theirs — the pace is set by how quickly they decide. **Deceased** → it is *the property*, never theirs; the will governs disposition rather than preference; we catalogue first and hand the inventory over; nothing moves without written authority; heirs on site get courtesy and no instruction is taken from them; they need not attend.
>
> **Home Cleanout is the one service that runs both ways, so intake asks outright.** Selecting *Home Cleanout* reveals a tick box — *"This is the property of someone who has died"* — and it is the only service that shows it: the three estate services are decedent work by definition, and downsizing, move and prep are living owners by definition. Ticking it switches the intake form the same way an estate service does (client name becomes *Deceased full name*, the header becomes *Deceased / Estate*, client phone and email are disabled) and reveals the **Authorized Representative** block. **It does not impose probate's mandatory fields** — the asterisks come off and the five-field block is not enforced — but it does require the rep's **last name plus a phone or an email**, because with the client's own contact fields disabled there is otherwise nobody to send the estimate to. It is stored as `job.decedent` and cleared whenever the service changes to one that cannot show it, so it cannot leak into the next intake.

> **Keep this separate from "does this job price a documentation step".** That question is answered by `JOB_STEPS` and decides whether a documentation *stage* exists at all. The living/deceased rule decides only the *voice*. A decedent Home Cleanout is written to the representative *without* ever claiming a court-grade inventory it is not selling. Collapsing the two would either sell documentation nobody bought or address a dead man as the client.
...
>
> **Downsizing keeps the decision-paced language, because there it is true.** Home Cleanout keeps a simpler middle version. A test asserts *Decisions, room by room* appears on downsizing and on neither estate nor probate.
...
>
> **This changed on 2026-08-03 and it changes what goes out on real jobs.** The routing used to be a list of four service types, so a **Home Cleanout of a house whose owner had just died** generated the *standard* agreement — a contract addressed throughout to a living owner of their own property, sent to an executor. Now a recorded **date of death** or a recorded **authorised representative** routes it to the estate form regardless of service type. *Any cleanout agreement issued before this date should be re-generated and re-read before it is relied on.*
