...
> **⚠ The Gmail draft link must carry a literal `@`.** Gmail resolves `/mail/u/<address>/` against the accounts signed in to that browser. Percent-encoding the `@` as `%40` matches no account and lands on *"Your account is not available"* — reported 2026-09-09, with a passing test over it that had asserted the encoded form. A malformed address now falls back to `/u/0/` rather than being pasted into a URL, and the draft link is also **printed under the buttons**, so a blocked popup or a browser signed in to several Google accounts never turns a draft that was successfully created into a dead end.
