...
**All three decedent services** — Estate Settlement, Probate and Contested Probate — require an authorized representative (executor, POA, administrator: name, role, phone, email) and a **date of death**. They are all offered the **estate attorney**, the *Who builds the inventory?* question and the two documentation gates below. **Probate** adds the court record on top: case number, the date the Letters issued, the §733.604 deadline computed from it, and whether the property must be sold — and there the attorney becomes **required**.
...
**Documentation level** — choose *Auto*, *Standard*, or *Formal — court & attorney grade*. **⚠ Auto does NOT resolve by service type alone, and has not since 2026-08-24** — it resolves from the two documentation gates below, which is why that control is often disabled with a reason beside it. Formal turns on the item-appraisal guardrails and makes chain-of-custody tracking mandatory. This is a *documentation* decision, kept separate from the Premium-estate *rate* toggle (§5c) — a premium job is not automatically a court-grade one, and vice-versa.

**Who builds the inventory?** (every estate service since 2026-09-21; new 2026-09-04) — *Havellin — inventory & estimated value*, *Havellin photographs & lists; counsel values*, or *Counsel's office — Havellin sorts & stages*. Ask the attorney at intake. It is stored on the job and **seeds** the estimate's *Documentation scope* (§5c); a fresh estimate opens at this answer instead of at Full.
