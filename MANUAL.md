...
**Documentation level** — choose *Auto*, *Standard*, or *Formal — court & attorney grade*. **⚠ Auto does NOT resolve by service type alone, and has not since 2026-08-24** — it resolves from the two documentation gates below **and, since 2026-09-21, from the engagement tier**, which is why that control is often disabled with a reason beside it. Formal turns on the item-appraisal guardrails and makes chain-of-custody tracking mandatory. This is a *documentation* decision, kept separate from the Premium-estate *rate* toggle (§5c) — a premium job is not automatically a court-grade one, and vice-versa.
...
> **Where the answer actually goes — two readers, and both of them withhold rather than add.** (1) The **Court Inventory** prints a block on its face on a matter recorded *Trust* or *Neither*, and never finalises or signs it (§10a). (2) The **client’s Drive workbook** drops *Letters Issued*, the *§733.604 Inventory Deadline* and the *Exempt §732.402* count from its Summary sheet, and so does the on-screen summary above it — the screen and the spreadsheet must never disagree about what a matter is. **Nothing else reads it yet**, and in particular the per-item **Asset Track still defaults to Probate on every line** whatever you answer here, so on a trust matter it is still set by hand (§10a). An unanswered matter type changes nothing anywhere.
...
The live readout under them names the level, why it is that level, and the two thresholds it moves — *Standard* lists individually above $1,000 and routes to a specialist at $3,000; *Formal* (Strict Mode) lists above $100.
...
- **Asset Track** — Probate / Trust / Non-probate / Homestead / Exempt (default Probate). Only Probate-track and Exempt-track items appear on the Court Inventory, so the probate schedule doesn't overstate the estate. **⚠ On a trust matter the default is wrong on every line** and the app cannot yet correct it — set the track by hand, and read the Court Inventory note below before printing one.
...
>
> **⚠ Nothing is carved out, and a room note never prints.** The Estate Inventory Report separates exempt, homestead and non-probate property because it is a schedule of what the estate *owns*. Which pot a chair falls into is a determination this engagement explicitly does not make — and the per-item Asset Track still defaults to *Probate* on every line — so carving here would assert a track nobody set, on the page handed to the person whose job it is to decide. Walkthrough notes are our working observation (*"family very sensitive about the study"*) and stay internal, the same rule that keeps the estimate worksheet out of a client’s folder.
>
...
> **⚠ The trustee's own schedule is not built yet, and this is not it.** That document needs the Chapter 736 citations, a successor-trustee signature block, the carve-out reversed so trust property is what is listed, and a line on its face stating it *supports* rather than constitutes a §736.08135 accounting — which is what keeps Havellin out of fiduciary accounting work. What shipped on 2026-09-21 stops the PROBATE document being signed on a matter with no probate in it. Until the trust schedule exists, hand counsel the **Estate Inventory Report** or the workbook instead.
...
| **Probate** | 4 | 17 | 11 | **11** — 9–12 by tier, **5** on a trust matter | The PR-authority box (which follows the *matter type* now, not the service), the *Letters of Administration on file* and *§733.604 inventory deadline* gate chips (both read from intake), and the *Florida court & legal compliance* list under Job Admin. |
