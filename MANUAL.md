...
The rooms are the **first thing under the Job Plan header**, above the phase accordion — one row per in-scope room in walkthrough order, carrying the walkthrough note from the estimate in grey italic, the shot counts (*2 found · 5 items · 1 after*) and a status pill. Rooms marked out of scope on the estimate are named in a single *Not in scope — homeowner is handling* line and are not rows. Tapping a row opens the **room workspace**, a full-screen page with three camera buttons (**As found · Items · After**), the **Lock** and **Cleared** taps, and a strip of every shot taken in the room grouped by pass, with Retry and discard on a failed one. The old per-room cards — two grids, five statuses, a typed name and category before every shot — are gone.

> **Why the rooms sit above the accordion and not inside Phase 1.** Every phase starts closed, and the first cut put the room list inside Phase 1 — so the whole field surface sat behind a tap on a bar reading *Phase 1*, which the browser check found when it could not click a room at all. The rooms are the work; the phases below them are the checklist.
...
**Tab: Job Plan** → select job. Generates once the estimate is approved. For labor-based services it opens with the room list and the room workspace (§10), then runs the phase playbook (Phase 0 Pre-Job → Phase 4 Close-Out), vendor & partner sourcing, and daily hours logging. For **Home Prep for Sale** it is the streamlined vendor/budget/checklist view described in Section 6c.

> **The room list sits above the phase accordion, in walkthrough order** — Entry & Living, then Kitchen & Utility, then Lifestyle Rooms, and so on: the same top-to-bottom as the Build Estimate grid and the client estimate, so a room can be found in one document by its position in the other. It is drawn once (rebuilt 2026-09-19, §10); the two phase grids are gone. A room saved without a grid position keeps its stored place at the end rather than dropping out of the plan.

> **⚠ A firearms flag at intake is a red banner across the top of the Job Plan (new 2026-09-19)** — *FIREARMS IN THIS HOUSE — flagged at intake*, the standing rule, what intake wrote down (or *Ticked at intake, no detail recorded — ask the client before Day 1*) and the protocol link. It is the first element in the header, above the client line and above the standing-flags brief, and a slimmer line of the same sits at the top of every room's workspace on that job. Nothing renders on a job intake did not flag. It also keeps the two firearms boxes on the Phase 1 checklist for that job, whatever the service type.
...
Asked directly (2026-09-11): *"does the job plan change for estate and non-estate jobs? … what do non estate jobs have?"* **It is one plan, not two.** Six of the seven services run the same phase spine and the estate rigor is *added* to it; only Home Prep for Sale has a different renderer. The 2026-09-19 cut took the checklist from roughly sixty boxes to the handful a person genuinely has to attest to, and replaced the rest with **derived lines** the app reads off the job record — agreement signed, deposit received, Letters on file, rooms locked, midpoint invoice sent and paid, vendors confirmed, release authority on every item leaving, rooms cleared, hours logged, change orders accepted, final invoice sent. A derived line is never ticked; it turns green when the record says so. Driven on the real renderer, same six-room house:

| Service | Phases | Boxes on the plan | Derived lines | Under Job Admin | What it adds over the one above it |
|---|---|---|---|---|---|
| **Home Editing** | 4 | 9 | 8 | 3 boxes | The baseline: the pre-job call, access and crew, the close-out and the three client asks. |
| **Home Transition** | **5** | 28 | 8 | 3 boxes | *New home prep* in Phase 0 and the whole **Phase 3 — Move Day** list — thirteen boxes, each of them real work a person attests to on the day. |
| **Home Cleanout** | 4 | 13 | 8 | 5 boxes | Pickups and disposal in Phase 2 (TC present for every valuables pickup; shredding), *home confirmed empty* at close-out; proceeds and settlement under Job Admin. |
| **Estate Settlement** | 4 | 17 | 9 | 5 boxes | The in-house documentation boxes in Phase 1 (firearms photographed where they lie; each checked for NFA; documents sequestered against a receipt; cash and jewelry logged) and the *estate attorney on file* and *written release authority* lines. |
| **Probate** | 4 | 18 | 11 | **12 boxes** | The PR-authority box, the *Letters of Administration on file* and *§733.604 inventory deadline* lines in Phase 0 (both read from intake), and the seven-item *Florida court & legal compliance* list under Job Admin. |
| **Contested Probate** | 4 | 18 | 11 | 12 boxes | **Nothing.** Identical to Probate, section for section — see the note below. |
| **Home Prep for Sale** | — | 6 | — | — | A different document entirely (§6c): vendor sourcing, budget and a coordination checklist. No phases, no room list, no hours log — that engagement bills no hours. |
