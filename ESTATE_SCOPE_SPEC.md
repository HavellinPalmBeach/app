...
Steps 1 to 6 shipped 2026-09-21. **Step 7 is next** — the trustee’s schedule: Chapter 736 citations, a successor-trustee signature block, the carve-out reversed so trust property is what is listed, and the *supports rather than constitutes a §736.08135 accounting* line. It is what makes the Court Inventory’s refusal on a trust matter actionable rather than a dead end, and it is the gate on `_invTrack`’s `'Probate'` default — which **must not** follow the matter type until that schedule exists. *Closes D9, D10.*
