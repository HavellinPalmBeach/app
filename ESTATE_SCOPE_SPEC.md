...
9. ~~**Either implement or retire `invListingThreshold`.**~~ **DONE 2026-09-22 — RETIRED.** The function and both constants (`INV_LISTING_THRESHOLD_STRICT` 100, `INV_LISTING_THRESHOLD_STANDARD` 1000) are deleted, and `docStandardEffect` now names what the level genuinely gates. *Closes D8.*
   ...
   ⚠⚠ **IMPLEMENTING IT WAS NOT A RENDERING CHANGE, WHICH IS WHY RETIRING IS THE ANSWER.** `_invScheduleSection` groups on the row's own `qty`, set at import time (itemise-or-lot), and a lot row carries ONE name, ONE value and ONE quantity — there is **no per-article data inside it** for a threshold to split. Any real floor is therefore a **capture-time gate on the inventory desk** (refuse or flag a lot line above it, while somebody is still there to split it), which is a separate build and a separate decision. §8 records that Florida sets no statutory itemisation floor, so nothing external forces it.
