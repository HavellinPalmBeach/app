...
Steps 1 to 7 shipped 2026-09-21. **Step 8 is next** — gate the estimate so Build Estimate will not price until the tier and the matter type are answered (*decision 3*). It is small: one gate, two fields that already exist. Then step 9, which is a decision rather than a build — `invListingThreshold` has exactly two references (its own definition and the sentence builder `docStandardEffect`), nothing enforces or displays an itemisation floor, and §8 records that **Florida sets no statutory itemisation floor** — the $100 / $1,000 are Havellin house rules. Retiring it and correcting the sentence is the recommendation; it is Anthony's call.
